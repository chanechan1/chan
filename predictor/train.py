...
def post_10():
    test_x = it._get_weathers_forecasts10("2023-11-13")  ##api 로 내일 데이터 따오기
    ...
    for errors in error_data:
        ...
        bounds = [(0.06, 1) for _ in range(len(errors))]
    ...
    ###################구한 최적의 가중치랑 구하는 것##################
    a = it._get_gen_forecasts10('2023-11-13')  ##api 로 내일 모델 입찰량
    a = pd.DataFrame(a)
    print(a)
    a = a[a.columns[1:]]
...
def post17():
    test_x = it._get_weathers_forecasts17("2023-11-13")  ##api 로 내일 데이터 따오기
    test_x = test_x[test_x.columns[1:]]  ##시간 땐거
    ...
    for errors in error_data:
        ...
        # bounds를 각 행의 오차 수에 맞게 설정
        bounds = [(0.02, 1) for _ in range(len(errors))]
    ...
    ###################구한 최적의 가중치랑 구하는 것##################
    a = it._get_gen_forecasts17('2023-11-13')  ##api 로 내일 모델 입찰량
    a = pd.DataFrame(a)
    print(a)
    a = a[a.columns[1:]]
    a.columns = optimal_weights_array.columns
