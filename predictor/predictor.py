...
def _10predictor():
    ...
    ###################test_x 실제 생산량을 알 수 있는 날짜의 그것을 넣기############################
    test_x = it._get_weathers_forecasts17('2023-11-02')  ##api 로 내일 데이터 따오기
    test_x = test_x[test_x.columns[1:]]  ##시간 땐거
    ...
    predictions = model.predict(test_x_reshaped)  ##이게 확률에 대한
    predictions = pd.DataFrame(predictions)  ##24*5의 데이터프레임으로 바꾸고
    b = it._get_gen_forecasts17('2023-11-02')  ##api 로 내일 모델 입찰량
    b = pd.DataFrame(b)
    b = b[b.columns[1:]]
    b.columns = predictions.columns
    predamount = b * predictions  ###가중치를 곱한 모델들을 섞은 예측발전량
    predamount = predamount.sum(axis=1).to_frame(name='predamounts')
