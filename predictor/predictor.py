...
def _10predictor():
    ...
    test_y=it._get_bids_result('2023-11-04') ##1102의 결과가 들어가 있는데 여기서 가장 큰 것만
    ...
    # 모델 학습
    model.fit(train_x_reshaped, train_y, epochs=150, batch_size=64,shuffle=1)
...
def _17pred():
    ...
    train_x = train_x.iloc[11568:]
    train_x = train_x[train_x.columns[2:]]  # 날씨실측정보
    ...
    ###################test_x 실제 생산량을 알 수 있는 날짜의 그것을 넣기############################
    test_x = it._get_weathers_forecasts17('2023-11-06')  ##api 로 내일 데이터 따오기
    test_x = test_x[test_x.columns[1:]] ##시간 땐거
    ...
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    # 모델 학습
    model.fit(train_x_reshaped, train_y, epochs=30, batch_size=32)
    ...
    b = it._get_gen_forecasts17('2023-11-06')  ##api 로 내일 모델 입찰량
    b= pd.DataFrame(b)
    ...
    b = pd.DataFrame(selected_values)

    indexes = list(range(len(selected_values)))

    # 플롯 생성
    plt.figure(figsize=(10, 5))  # 플롯 크기 설정
    plt.plot(indexes, selected_values, marker='o', linestyle='-')  # 선 그래프와 마커로 표시
